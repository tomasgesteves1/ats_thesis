...
# ==============================================================================
# SDF Parameters with Added Mass & Coriolis formulation
# ==============================================================================
# Total Mass: Base (227) + GPS (1.0) + IMU (0.1) + Left Engine (15.0) + Right Engine (15.0) + Propellers (2 * 0.5)
MASS = 259.1          

# Total Yaw Inertia: Base IZZ (495.037) + Engine IZZs (2 * 0.078) 
# + Parallel Axis Theorem contribution of engines: 2 * 15.0 * ((-2.373776)^2 + 1.027135^2)
IZZ = 695.9           

# Added Mass estimations (Fossen Hydrodynamics)
X_DOT_U = 0.2 * MASS   # Added mass in Surge (approx 20%)
Y_DOT_V = 0.7 * MASS   # Added mass in Sway (approx 70%)
N_DOT_R = 0.4 * IZZ    # Added inertia in Yaw (approx 40%)
...
X_UU_BWD = 150.0       # Quadratic surge damping (backward - symmetric in SDF)
...
def wamv_dynamics_numerical(x, u_virtual):
    ...
    # Dynamics (Fossen 3-DOF model equations with Coriolis and Added Mass)
    x_uu_eff = X_UU_BWD if u < 0.0 else X_UU
    
    u_dot = (X_force - (X_U + x_uu_eff * np.abs(u)) * u) / M_X + (M_Y / M_X) * v * r
    v_dot = (Y_force - (Y_V + Y_VV * np.abs(v)) * v) / M_Y - (M_X / M_Y) * u * r
    r_dot = (N_moment - (N_R + N_RR * np.abs(r)) * r - (M_Y - M_X) * u * v) / J_Z
...
def get_wamv_casadi_model():
    ...
    f_expl = ca.vertcat(
        ...
        r,
        (X_force - (X_U + x_uu_eff * ca.fabs(u)) * u) / M_X + (M_Y / M_X) * v * r,
        (Y_force - (Y_V + Y_VV * ca.fabs(v)) * v) / M_Y - (M_X / M_Y) * u * r,
        (N_moment - (N_R + N_RR * ca.fabs(r)) * r - (M_Y - M_X) * u * v) / J_Z
    )
