...
\addto\captionsenglish{\renewcommand{\acronymname}{Acronyms}} % Acronyms
...
% report.cls defaults to \flushbottom, which stretches interline glue so every
% page reaches the bottom margin. On pages with little text (short paragraphs
% next to equations/figures), that stretch piles up in a single gap, usually
% right before a heading. \raggedbottom lets pages end short instead.
\raggedbottom
...
% TikZ Package for vector graphics
\usepackage{tikz}
\usetikzlibrary{arrows.meta,calc,3d}

% Packages and custom command to track reviewed sections/paragraphs
\usepackage{xcolor}
\usepackage{marginnote}
\usepackage{pifont}
\usepackage{etoolbox}
\makeatletter
\appto{\@mn@margintest}{\@tempswatrue}
\makeatother
% Toggle to hide the review checkmarks (set to \showreviewmarksfalse before
% compiling the version handed to the advisors / for submission)
\newif\ifshowreviewmarks
\showreviewmarkstrue
\newcommand{\reviewed}{\ifshowreviewmarks\marginnote{\textcolor{green}{\ding{51}}}\fi}
