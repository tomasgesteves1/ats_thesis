...
With $\mathbf{x}_v$ and $\mathbf{u}_v$ defined, the dynamics $\dot{\mathbf{x}}_v = f(\mathbf{x}_v,\mathbf{u}_v)$ follow directly.
...
The input $\mathbf{u}_a = [T,\phi,\theta,\psi]^T$ of the vehicle model defined in Chapter~\ref{chapter:modeling} commands the thrust and the attitude directly, assuming each of them is reached instantaneously.
...
\begin{equation}
    \mathbf{x}_a = [x,y,z,\dot x,\dot y,\dot z,\ \phi,\theta,\psi]^T \in \mathbb{R}^{9}.
...
\begin{equation}
    \mathbf{u}_a = [\dot\phi,\dot\theta,\dot\psi,T]^T \in \mathbb{R}^4.
    \label{eq:uav_mpc_input_augmented}
\end{equation}

With $\mathbf{x}_a$ and $\mathbf{u}_a$ defined, the dynamics $\dot{\mathbf{x}}_a = f(\mathbf{x}_a,\mathbf{u}_a)$ follow directly.
...
The translational dynamics follow \eqref{eq:uav_dynamics}, with the thrust term expanded and the tether force $\mathbf{F}_{\mathrm{tether}}^{\mathcal{I}} = [F_{\mathrm{tether},x}^{\mathcal{I}}, F_{\mathrm{tether},y}^{\mathcal{I}}, F_{\mathrm{tether},z}^{\mathcal{I}}]^T$ given by \eqref{eq:uav_tether_geometry}--\eqref{eq:uav_tether_force},
\begin{equation}
...
Finally, $\dot\phi,\dot\theta,\dot\psi$ appear directly as the last three components of $\mathbf{u}_a$,
\begin{equation}
...
The next term penalizes the yaw error, but rather than tracking a fixed yaw angle, $\psi$ is aligned with the direction to a reference point $\mathbf{p}_{d,ref} = [x_{d,ref},y_{d,ref}]^T$, itself possibly varying along the horizon,
\begin{equation}
    \mathbf{d} = \mathbf{p}_{d,ref} - \mathbf{p}_{xy},
    \label{eq:uav_mpc_yaw_direction}
\end{equation}
where $\mathbf{p}_{xy}=[x,y]^T$. Since $\psi$ is an angle, the same periodicity used in \eqref{eq:usv_mpc_cost_heading} applies, expressed here as an alignment between $\psi$ and $\mathbf{d}$ rather than a difference between two angles,
\begin{equation}
    q_\psi\left(1 - \frac{\cos\psi\,d_x + \sin\psi\,d_y}{\|\mathbf{d}\|}\right).
    \label{eq:uav_mpc_cost_yaw}
\end{equation}
This term is minimized when $\psi$ points along $\mathbf{d}$, the direction to the reference point $\mathbf{p}_{d,ref}$.

The last term penalizes the input $\mathbf{u}_a$. Penalizing $T$ toward zero would fight the physics of hover, where thrust must counteract gravity rather than vanish. To keep this term a measure of control effort rather than a penalty on the thrust needed just to hover, the input is tracked against a reference $\mathbf{u}_{a,ref} = [0,0,0,mg]^T$, using $mg$ as a nominal hover approximation,
\begin{equation}
...
\begin{equation}
    \begin{aligned}
        \ell(\mathbf{x}_a,\mathbf{u}_a) = {} & \|\mathbf{p}-\mathbf{p}_{ref}\|^2_{Q_p} + \|\mathbf{v}-\mathbf{v}_{ref}\|^2_{Q_v} + \big\|[\phi,\theta]^T\big\|^2_{Q_{tilt}} \\
        & + q_\psi\left(1 - \frac{\cos\psi\,d_x + \sin\psi\,d_y}{\|\mathbf{d}\|}\right) + \|\mathbf{u}_a-\mathbf{u}_{a,ref}\|^2_R.
    \end{aligned}
...
\begin{equation}
    \ell_f(\mathbf{x}_a) = \|\mathbf{p}-\mathbf{p}_{ref}\|^2_{Q_p} + \|\mathbf{v}-\mathbf{v}_{ref}\|^2_{Q_v} + \big\|[\phi,\theta]^T\big\|^2_{Q_{tilt}} + q_\psi\left(1 - \frac{\cos\psi\,d_x + \sin\psi\,d_y}{\|\mathbf{d}\|}\right).
    \label{eq:uav_mpc_terminal_cost}
\end{equation}

The velocity and tilt are each bounded, within the vehicle's operating envelope. Since the horizontal and vertical velocity are limited by different physical factors, the vehicle's speed is bounded separately in each,
\begin{equation}
    \big\|[\dot x,\dot y]^T\big\| \le v_{xy,max}, \quad |\dot z| \le v_{z,max}, \quad |\phi| \le \phi_{max}, \quad |\theta| \le \theta_{max}.
...
A soft constraint avoids this by allowing a bound to be exceeded, at a cost. Each bound in \eqref{eq:uav_mpc_state_bounds} is loosened by its own slack $\sigma_j\ge0$, $j=1,\dots,4$, and a term
\begin{equation}
    \rho(\sigma) = z\sigma + Z\sigma^2, \qquad z,Z\ge0,
    \label{eq:uav_mpc_slack_penalty}
\end{equation}
is added to the cost for each one, penalizing $\sigma_j>0$ while still preferring $\sigma_j=0$ whenever the original bound can be met on its own.

The attitude rate and the thrust remain bounded as hard inputs,
\begin{equation}
...
The tether limits the distance between the vehicle and its attachment point on the vessel, at position $\mathbf{p}_v = [x_v,y_v,0]^T$, to its maximum length $L_{max}$. Since the vessel is itself being predicted by the USV NMPC, $\mathbf{p}_v$ is received at each node as the vessel's predicted position there. This constraint is softened the same way, with its own slack $\sigma_5\ge0$ weighted more heavily by \eqref{eq:uav_mpc_slack_penalty}, keeping it close to strictly enforced,
\begin{equation}
    g(\mathbf{x}_a) = (x-x_v)^2 + (y-y_v)^2 + z^2 - L_{max}^2 \le \sigma_5.
    \label{eq:uav_mpc_tether_constraint}
\end{equation}

Collecting \eqref{eq:uav_mpc_state_bounds} and \eqref{eq:uav_mpc_tether_constraint}, with their slacks stacked as $\boldsymbol\sigma=[\sigma_1,\dots,\sigma_5]^T\ge\mathbf{0}$, gives the soft constraint set $\mathcal{X}_\sigma$ of admissible pairs $(\mathbf{x}_a,\boldsymbol\sigma)$, and the aggregate penalty
\begin{equation}
    P(\boldsymbol\sigma) = \sum_{j=1}^{5} \rho(\sigma_j).
...
\begin{equation}
    \hat{\mathbf{x}}_{a,k|k} = \big[x_k,y_k,z_k,\dot x_k,\dot y_k,\dot z_k,\ \phi_k,\theta_k,\psi_k\big]^T.
    \label{eq:uav_mpc_initial_condition}
\end{equation}

Collecting the cost, the model, and the constraints gives the optimal control problem the UAV solves at every sampling instant, with the slack vector $\hat{\boldsymbol\sigma}_{k+i|k}$ entering as an additional decision variable,
\begin{align}
    ...
    \text{s.t.} \quad & \hat{\mathbf{x}}_{a,k|k} = \big[x_k,y_k,z_k,\dot x_k,\dot y_k,\dot z_k,\phi_k,\theta_k,\psi_k\big]^T, \notag \\
...
\end{align}
where $F$ is the one-step map obtained by discretizing $f(\cdot)$. The vehicle receives the first input $\hat{\mathbf{u}}_{a,k|k}$ of the solution, applying it over one sampling interval.