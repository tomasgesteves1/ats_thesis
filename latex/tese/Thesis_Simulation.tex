...
A flexible tether, managed by an active winch system on the catamaran's deck, physically couples the WAM-V catamaran and the x500 quadrotor.
    ...
    The active winch is mounted on the WAM-V deck, offset by $\mathbf{p}_{\mathrm{anchor}, v} = [-0.8, 0.0, 0.0]^\top\,\text{m}$ from the center of gravity. The winch dynamically spools the tether to adjust its unstretched length $L_{tether}$ between $1.5\,\text{m}$ and $50.0\,\text{m}$, with a maximum spooling speed of $10.0\,\text{m/s}$.
...
To run and coordinate the simulation, a co-simulation environment is built in Gazebo Harmonic \cite{koenig2004gazebo}. The software stack integrates the simulator with the control pipeline through communication bridges, as shown in Figure~\ref{fig:software_architecture}.
    ...
    \begin{itemize}
        \item \textbf{USV Control Loop (10 Hz):} The Nonlinear Model Predictive Control (NMPC) node receives the vessel's ground-truth odometry and calculates the virtual forces and moment $[X, Y, N]^\top$. The thruster allocation block maps these virtual forces to steering angles and motor thrust commands published to Gazebo.
