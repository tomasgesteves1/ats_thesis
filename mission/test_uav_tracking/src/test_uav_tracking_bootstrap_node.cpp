...
class TestUavTrackingBootstrapNode : public rclcpp::Node {
public:
    TestUavTrackingBootstrapNode() : Node("test_uav_tracking_bootstrap_node"), state_(UavState::STANDBY) {
        auto qos = rclcpp::SensorDataQoS();
        status_sub_ = this->create_subscription<px4_msgs::msg::VehicleStatus>(
            "/px4_1/fmu/out/vehicle_status_v1", qos,
            std::bind(&TestUavTrackingBootstrapNode::statusCallback, this, std::placeholders::_1)
        );
        ...
        timer_ = this->create_wall_timer(
            ...
            std::bind(&TestUavTrackingBootstrapNode::controlLoop, this)
        );
        
        RCLCPP_INFO(this->get_logger(), "Test UAV Tracking Bootstrap Node (MPC Takeoff) initialized.");
    }
...
private:
    ...
    void controlLoop() {
        ...
        switch (state_) {
            ...
            case UavState::DONE: {
                RCLCPP_INFO(this->get_logger(), "Shutting down marsupial bootstrap node.");
                rclcpp::shutdown();
                break;
            }
        }
...
int main(int argc, char** argv) {
    ...
    auto node = std::make_shared<test_uav_tracking::TestUavTrackingBootstrapNode>();
    rclcpp::spin(node);
    return 0;
}
