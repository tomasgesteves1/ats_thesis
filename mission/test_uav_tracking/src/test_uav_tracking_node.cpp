...
TestUavTrackingNode::TestUavTrackingNode() 
      ...
      latest_boat_odom_(nullptr)
{
    ...
    pipeline_ = std::make_unique<TestUavTrackingPipeline>();
    ...
    boat_odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "boat_odom", qos, 
        std::bind(&TestUavTrackingNode::boatOdomCallback, this, std::placeholders::_1)
    );
    ...
    timer_ = this->create_wall_timer(
        ...
        std::bind(&TestUavTrackingNode::timerCallback, this)
    );

    RCLCPP_INFO(this->get_logger(), "Test UAV Tracking Node initialized.");
}

void TestUavTrackingNode::boatOdomCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
    latest_boat_odom_ = msg;
}

void TestUavTrackingNode::timerCallback() {
