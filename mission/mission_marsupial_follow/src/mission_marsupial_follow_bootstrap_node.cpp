...
class MarsupialBootstrapNode : public rclcpp::Node {
public:
    MarsupialBootstrapNode() : Node("marsupial_bootstrap_node"), state_(UavState::STANDBY) {
        auto qos = rclcpp::SensorDataQoS();
        status_sub_ = this->create_subscription<px4_msgs::msg::VehicleStatus>(
            "/px4_1/fmu/out/vehicle_status_v1", qos,
            std::bind(&MarsupialBootstrapNode::statusCallback, this, std::placeholders::_1)
        );
        ...
        timer_ = this->create_wall_timer(
            ...
            std::bind(&MarsupialBootstrapNode::controlLoop, this)
        );
        
        RCLCPP_INFO(this->get_logger(), "Marsupial Follow Bootstrap Node (MPC Takeoff) initialized.");
    }
...
int main(int argc, char** argv) {
    ...
    auto node = std::make_shared<mission_marsupial_follow::MarsupialBootstrapNode>();
    rclcpp::spin(node);
    return 0;
}
