...
MissionMarsupialFollowNode::MissionMarsupialFollowNode() 
      ...
      latest_boat_odom_(nullptr)
{
    ...
    pipeline_ = std::make_unique<MissionMarsupialFollowPipeline>();
    ...
    boat_odom_sub_ = this->create_subscription<nav_msgs::msg::Odometry>(
        "boat_odom", qos, 
        std::bind(&MissionMarsupialFollowNode::boatOdomCallback, this, std::placeholders::_1)
    );
    ...
    timer_ = this->create_wall_timer(
        ...
        std::bind(&MissionMarsupialFollowNode::timerCallback, this)
    );

    RCLCPP_INFO(this->get_logger(), "Mission Marsupial Follow Node initialized.");
}

void MissionMarsupialFollowNode::boatOdomCallback(const nav_msgs::msg::Odometry::SharedPtr msg) {
    latest_boat_odom_ = msg;
}

void MissionMarsupialFollowNode::timerCallback() {
    ...
    double boat_z = 0.0; // Treat boat as 2D
